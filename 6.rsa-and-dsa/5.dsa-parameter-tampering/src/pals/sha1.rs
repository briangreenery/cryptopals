...
impl Hasher {
    ...
    pub fn from(state: &[u8; 20], data_len: usize) -> Hasher {
        ...
        for (i, byte) in state.iter().enumerate() {
            hasher.state[i / 4] = (hasher.state[i / 4] << 8) | (*byte as u32);
        }
    ...
    pub fn end(&mut self) -> [u8; 20] {
        ...
        for (bucket, value) in self.state.iter().enumerate() {
            for byte in 0..4 {
                result[4 * bucket + byte] = ((value >> (24 - (8 * byte))) % 256) as u8;
            }
        }
...
fn hmac_pad(key: &[u8], pad_byte: u8) -> [u8; 64] {
    let mut buffer = [pad_byte; 64];

    for (i, key_byte) in key.iter().enumerate() {
        buffer[i] = key_byte ^ pad_byte;
    }

    buffer
}

fn hmac_round(key: &[u8], pad_byte: u8, data: &[u8]) -> [u8; 20] {
    let mut hasher = Hasher::new();

    if key.len() <= 64 {
        hasher.write(&hmac_pad(key, pad_byte));
    } else {
        hasher.write(&hmac_pad(&hash(key), pad_byte));
    }
...
pub fn hmac(key: &[u8], data: &[u8]) -> [u8; 20] {
    hmac_round(key, 0x5C, &hmac_round(key, 0x36, data))
}
