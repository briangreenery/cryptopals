...
fn sha1_pad(message: &[u8], key_len: usize) -> Vec<u8> {
    ...
    for i in 0..8 {
        padded.push(((bit_len >> (56 - (8 * i))) % 256) as u8);
    }
...
fn main() {
    ...
    for key_len in 0..1000 {
        let padded = sha1_pad(message, key_len);
        ...
        let mut forger = pals::sha1::Hasher::from(&hash, key_len + padded.len());
        forger.write(admin);
